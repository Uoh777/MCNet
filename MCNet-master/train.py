...
for name, param in model.named_parameters():
        if 'resnet_image' in name or 'res_thermal' in name:
            print(name)
            base.append(param)
        else:
            print(name)
            head.append(param)
optimizer = torch.optim.SGD([{'params': base}, {'params': head}], lr=opt.lr, momentum=opt.momentum,
                                weight_decay=opt.decay_rate, nesterov=True)


#set the path
train_root = opt.train_data_root
test_root = opt.test_data_root
...
def train(train_loader, model, optimizer, epoch,save_path):
    ...
    try:
        ...
        if (epoch) % 5 == 0:
            torch.save(model.state_dict(), save_path+'MCNet_epoch_{}.pth'.format(epoch))
    ...
    with torch.no_grad():
        ...
        if epoch==1:
            ...
            if mae<best_mae:
                best_mae=mae
                best_epoch=epoch
                torch.save(model.state_dict(), save_path+'MCNet_epoch_best.pth')
